...
- Cursor registration docs in `docs/INTEGRATION.md` (Agent/Composer mapping, `~/.claude/settings.json` read-through / double-fire trap, `sessionStart` startup race, one-machine registration table, troubleshooting), pinned as version-dependent observations against Cursor 3.12 (July 2026). `AGENTS.md` and `README.md` name Cursor hooks alongside Claude Code and Codex.
