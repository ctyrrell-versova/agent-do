...
- agent-ci: `--repo owner/repo` (or `--repo=…`) is now a global option honoured by every verb, stripped alongside `--json` before dispatch and preferred by `get_repo()` over the origin remote. Previously only `triage` parsed it; `runs --repo X` took `--repo` as the workflow name and silently listed the origin fork's runs — from a fork checkout that meant the wrong repository's CI with no indication. A dangling or malformed `--repo` now exits 2. New `tests/test_ci_repo_flag.py` runs every verb against a recording fake `gh` from a directory with no git remote.
