...
- agent-gh: new `create` verb opens a pull request — `--repo`, `--base`, `--head`, `--title`, `--body`/`--body-file` (`-` for stdin), `--draft`, `--reviewer`, `--label`, `--assignee`, `--json`. Non-interactive by construction: `--title` and a body are required and refused before `gh` is invoked, so the verb can never fall into `gh`'s editor prompt inside an agent session. Reads the PR back with `gh pr view --json` and reports url/number/head/base. Closes the most-hit gap in the tool: seven documented fallbacks to raw `gh pr create` since July, seven more today. Registry: `create` added to the interact contract with a routing example; `harness inspect` clean. New `tests/test_gh_create.py` runs the verb against a recording fake `gh`.
