...
- agent-render: `show` and `deploys` honour the global `--json` (both silently ignored it and printed text). `show` now prints `Root dir`, `Auto-deploy` with its trigger, and the build-filter paths/ignored lists — the three fields that decide whether a push deploys at all (a service missed five promotions in Sept 2026 because nothing surfaced its `buildFilter`) — and prints the service URL before the repo link so a first-URL grep no longer returns github.com. `deploys` prints the full commit id instead of an 8-char prefix, so live-commit equality can be proven from the listing. Field names verified against the live `/services/{id}` and `/deploys` responses. New `tests/test_render_show_deploys.py`.
