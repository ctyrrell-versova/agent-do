...
### Added
- Cursor integration: `hooks/cursor/` adapters translate Cursor hook JSON to the canonical Claude hooks; `install.sh --cursor` copies adapters to `~/.cursor/hooks/` and templates `~/.cursor/hooks.json`.
- `INTEGRATION.md` now documents Cursor Agent (Composer) hook wiring, duplicate-hook avoidance, the `~/.claude/settings.json` discovery path, and the `sessionStart` / `MainThreadShellExec` startup race.
