...
- agent-render: `db show`, `db connect-info`, and `kv connect-info` mask passwords by default — URI userinfo (`postgresql://`, `redis://`, `rediss://`), `PGPASSWORD=` in `psqlCommand`, `REDISCLI_AUTH=` in `cliCommand`, and the bare `password` field the API returns — and print the raw values only with `--reveal`, mirroring `creds get --reveal`. `db show --json` and `db connect-info --json` now emit JSON (previously the global flag was silently ignored on both); `db show --json` wraps `{postgres, connectionInfo}`. `db show` also stops interpolating API JSON into a Python source string (passes it via environment). Motivation: every unmasked call landed a production DB password in an agent transcript (six occurrences Jul–Sep 2026). New `tests/test_render_db_secrets.py` drives the real `cmd_*` functions against a stubbed API and fails on the pre-change tool.
