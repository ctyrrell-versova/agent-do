...
- agent-render: `resolve_service` treats `crn-*` (cron job) IDs as already-resolved, alongside `srv-*`. Previously any `crn-` ID fell through to a name lookup and every read verb (`show`, `deploys`, `deploy show`, `events`, `jobs`, `logs`, …) reported "Service not found" for a cron whose name resolved fine — which on 2026-09-10 produced the false conclusion that cron jobs have no deploy list. `/services` only issues `srv-` and `crn-` prefixes (workers are `srv-`), so this is the complete set. New `tests/test_render_resolve.py`.
