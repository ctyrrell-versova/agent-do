...
- agent-betterstack: `heartbeats --json` redacts each heartbeat's ping URL token (`…/heartbeat/<redacted>`) unless `--reveal` — the URL is the write credential; anyone holding it can mark the job alive — and the table drops the `LAST PING` column, which the v2 list endpoint never populates (`last_pinged_at` is absent from its attributes), in favour of `PERIOD`/`GRACE`. New `tests/test_betterstack_heartbeats.py`.
