...
- agent-render: `render_request_checked` no longer sets a `RETURN` trap to remove its temp file. Bash RETURN traps are not function-scoped, so the trap fired a second time when the *calling* verb returned, after the helper's `local tmp_body` was gone — `tmp_body: unbound variable` on stderr after every otherwise-successful direct call (`jobs`, `cron run`, `env-group delete`, `blueprint delete`, `kv list`, … — five sightings Jul–Sep 2026), and under a caller's `set -e` it aborted the script before the success message. The temp file is now removed explicitly on both paths; a curl transport failure returns 1 from the helper instead of relying on `set -e`. New `tests/test_render_request_trap.py` reproduces the leak with a stubbed curl.
