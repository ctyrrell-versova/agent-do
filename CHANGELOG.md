...
- agent-render: `blueprint validate` sends what the API actually requires — `multipart/form-data` with `ownerId` (workspace id; defaults to the first workspace, `--owner` to pick) and `file` (the render.yaml). It previously POSTed a JSON body under a guessed key and the endpoint answered HTTP 400 `Content-Type isn't multipart/form-data` on every call (three sightings Sept 2026). Output is now `valid:`, each error with line:col and path, and the plan's action count; exit 0 valid, 2 invalid, `--json` for the raw response. Contract and response shape verified against api-docs.render.com and live calls. New `render_request_multipart` helper. New `tests/test_render_blueprint_validate.py`.
