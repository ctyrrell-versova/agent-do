...
- agent-render: `db create`, `kv create`, `env-group create`, `project create`, and `environment create` no longer accept a `-`-prefixed token as the resource name. `kv create --help` previously POSTed a live Key Value instance named `--help` (observed 2026-07-13, instance deleted by hand); any mistyped flag on the other four would have done the same. `--help`/`-h` now print the verb's usage and exit 0; any other unknown flag errors with the usage on stderr and exits 1 before any API call. `tests/test_render_create_argparse.py` runs every create branch against a stub that fails the test on any request.
